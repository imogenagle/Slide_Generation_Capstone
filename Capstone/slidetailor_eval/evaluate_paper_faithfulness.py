#!/usr/bin/env python3
"""Paper-faithfulness evaluation for SlideGen decks."""
...
DEFAULT_PROMPT_PATH = REPO_ROOT / "utils" / "prompt_templates" / "slidetailor_eval" / "paper_faithfulness.yaml"


def evaluate_paper_faithfulness(
    *,
    pptx_path: Path,
    ...
    per_slide = evaluate_single_slide_images(
        metric_name="paper_faithfulness",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate paper faithfulness.")
    ...
    add_shared_args(parser, metric_name="paper_faithfulness")
    args = parser.parse_args()

    load_runtime_env()
    slide_dir = args.slide_image_dir or args.pptx_path.with_suffix("")
    if args.slide_image_dir is None:
        slide_dir = slide_dir.parent / f"{slide_dir.name}_paper_faithfulness_images"
    ...
    output_path = resolve_output_path("paper_faithfulness", args.output, args.pptx_path.stem)
